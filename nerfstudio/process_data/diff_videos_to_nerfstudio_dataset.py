...
@dataclass
class DiffVideosToNerfstudioDataset(ColmapConverterToNerfstudioDataset):
    ...
    def main(self) -> None:  # pylint: disable=R0915
        ...
        if self.camera_type == "equirectangular":
            ...
            fullground_path = list(self.data.glob('*whole.mp4'))[0]
            background_path = list(self.data.glob('*background.mp4'))[0]

            num_fullground_frames_target = self.num_frames_target // 2
            num_background_frames_target = self.num_frames_target - num_fullground_frames_target

            fullground_img_dir = self.output_dir / "images_fullground"
            background_img_dir = self.output_dir / "images_background"

            os.makedirs(fullground_img_dir, exist_ok=True)
            os.makedirs(background_img_dir, exist_ok=True)

            # Extract fullground images
            summary_log, num_extracted_fullground_frames = process_data_utils.convert_video_to_images(
                fullground_path,
                image_dir=fullground_img_dir,
                num_frames_target=num_fullground_frames_target,
                crop_factor=self.crop_factor,
                verbose=self.verbose,
            )

            # Extract background images
            summary_log, num_extracted_background_frames = process_data_utils.convert_video_to_images(
                background_path,
                image_dir=background_img_dir,
                num_frames_target=num_background_frames_target,
                crop_factor=self.crop_factor,
                verbose=self.verbose,
            )

            # Copy images
            for frame in os.listdir(fullground_img_dir):
                frame_path = fullground_img_dir / frame
                shutil.copy(frame_path, self.image_dir / frame)

            for frame in os.listdir(background_img_dir):
                frame_path = background_img_dir / frame
                new_index = int(frame[6:11])+num_extracted_fullground_frames
                new_frame_name = "frame_%05d.png" % new_index
                shutil.copy(frame_path, self.image_dir / new_frame_name)

            shutil.rmtree(fullground_img_dir)
            shutil.rmtree(background_img_dir)

            # Save highest fullground index
            with open(self.output_dir / "highest_fullground_idx.txt", 'w') as f:
                f.write("%d" % num_extracted_fullground_frames)
